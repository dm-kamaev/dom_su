...
const { staffUrl, isMobileVersion, staffTemplate } = require('./utils');
...
// /staff/credits_now/f82270ac-e9b0-11e5-80de-00155d594900/
let getCreditsCurrent = loginRequired(getEmployeeHeader(async function(ctx, next, request1C, GetEmployeeData, templateCtx) {
  ...
  //     }
  //   }, {
  //     "Date": "2018-01-16T13:27:18Z",
  //     "Description": "Вычет по рекламации",
  //     "Sum": -3000,
  //     "Document": {
  //       "Number": "1521751-00001",
  //       "Description": "Порча"
  //     }
  //   }],
  //   MonthlyCharges: [],
  //   Sum: 3696
  // }
  // GetCurrentWageForEmployee.response.InterestOnOrder = [{
  //     "Date": new Date,
  //     "Sum": 13000,
  //     "Description": "Основание\Описание",
  //     "DepartureID": 12345,
  //     "Metro": "Метро",
  //     "Client": 'Петя',
  //     "Adjustment": {
  ...
  // ];
  const res_GetCurrentWageForEmployee = GetCurrentWageForEmployee.response;
  ...
  const additional_charges = res_GetCurrentWageForEmployee.AdditionalCharges || [];
  templateCtx.additional_charges = additional_charges.map(function (charge) {
    const doc = charge.Document || {};
    const date = charge.Date || '';
    return {
      date: (date) ? moment.utc(date).format('HH:mm') : '-',
      description: charge.Description || '', // общее описание нарушения
      sum: charge.Sum || 0,
      document: {
        number: doc.Number || '', // номер нарушения, документ
        description: doc.Description || '-', // описание докумета нарушения
      }
    };
  });
...
let ajaxDepositList = loginRequired(async function (ctx){
  let templateCtx = {};
  let countItem = 5;
...
let ajaxCreditsList = loginRequired(async function (ctx){
  let templateCtx = {};
  let countItem = 5;
  let fromItem = Number(ctx.query.item_count);
  const request1C = new Request1C(ctx.state.pancakeUser.auth1C.token, ctx.state.pancakeUser.uuid, '', '', true, ctx);
